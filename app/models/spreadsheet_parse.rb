...
class SpreadsheetParse
    ...
    CSV.foreach(csv_file_paths[:orgs_csv_file], headers: :first_row) do |org_row|
      unless organzation_already_exists?(org_row['name'])
        ...
        if new_organization.save!(validate: false)
          ...
          create_locations_and_location_services(csv_file_paths[:locations_csv_file],
                                                 csv_file_paths[:location_services_csv_file], new_organization, org_row['id'])
        end
      end
    end
  end
  ...
  def create_locations_and_location_services(locations_csv_file_path, location_services_csv_file_path, new_organization, org_id)
